...
def _apply_quantization(method: str) -> tuple[float, str]:
    ...
    if method == "int8":
        ...
        BitsAndBytesConfig(load_in_4bit=True, bnb_4bit_compute_dtype=mlx.float16, bnb_4bit_use_double_quant=True)
    ...
    if mlx is not None and BitsAndBytesConfig is not None and (AutoModelForCausalLM is not None):
        try:
            ...
            if status.startswith("unsupported"):
                return {"backend": "mlx", "model": model_name, "method": method, "status": status, "memory_reduction_factor": 0.0}
