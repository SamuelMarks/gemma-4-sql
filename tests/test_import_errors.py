...
def test_missed_direct():
    ...
    from unittest import mock
    with mock.patch.dict(sys.modules, {"jax.nn": None}):
        if "gemma_4_sql.backends.jax.dpo" in sys.modules: del sys.modules["gemma_4_sql.backends.jax.dpo"]
        try:
    ...
    with mock.patch.dict(sys.modules, {"orbax.checkpoint": None}):
        if "gemma_4_sql.backends.jax.export" in sys.modules: del sys.modules["gemma_4_sql.backends.jax.export"]
        try:
            import gemma_4_sql.backends.jax.export as exp
            exp.export_model("foo", "bar")
        except Exception: pass
    ...
    with mock.patch.dict(sys.modules, {"flax.nnx": None}):
        if "gemma_4_sql.backends.jax.inference" in sys.modules: del sys.modules["gemma_4_sql.backends.jax.inference"]
        try:
    ...
    with mock.patch.dict(sys.modules, {"jax.numpy": None}):
        if "gemma_4_sql.backends.jax.quantize" in sys.modules: del sys.modules["gemma_4_sql.backends.jax.quantize"]
        try:
    ...
    with mock.patch.dict(sys.modules, {"flax.nnx": None}):
        if "gemma_4_sql.backends.jax.train" in sys.modules: del sys.modules["gemma_4_sql.backends.jax.train"]
        try:
    ...
    with mock.patch.dict(sys.modules, {"keras_nlp": None}):
        if "gemma_4_sql.backends.keras.export" in sys.modules: del sys.modules["gemma_4_sql.backends.keras.export"]
        try:
            import gemma_4_sql.backends.keras.export as exp
            exp.export_model("foo", "bar")
        except Exception: pass
    ...
    with mock.patch.dict(sys.modules, {"tensorflow": None}):
        if "gemma_4_sql.backends.keras.inference" in sys.modules: del sys.modules["gemma_4_sql.backends.keras.inference"]
        try:
            import gemma_4_sql.backends.keras.inference as inf
            inf.generate_sql("foo", "bar")
        except Exception: pass
    ...
    with mock.patch.dict(sys.modules, {"tensorflow": None}):
        if "gemma_4_sql.backends.keras.train" in sys.modules: del sys.modules["gemma_4_sql.backends.keras.train"]
        try:
            import gemma_4_sql.backends.keras.train as tr
            tr.train_model("foo", "bar")
        except Exception: pass
    ...
    with mock.patch.dict(sys.modules, {"maxtext.models.gemma4": None}):
        if "gemma_4_sql.backends.maxtext.export" in sys.modules: del sys.modules["gemma_4_sql.backends.maxtext.export"]
        try:
            import gemma_4_sql.backends.maxtext.export as exp
            exp.export_model("foo", "bar")
        except Exception: pass
    ...
    with mock.patch.dict(sys.modules, {"safetensors.torch": None}):
        if "gemma_4_sql.backends.pytorch.export" in sys.modules: del sys.modules["gemma_4_sql.backends.pytorch.export"]
        try:
            import gemma_4_sql.backends.pytorch.export as exp
            exp.export_model("foo", "bar")
        except Exception: pass
    ...
    with mock.patch.dict(sys.modules, {"torch": None}):
        if "gemma_4_sql.backends.pytorch.inference" in sys.modules: del sys.modules["gemma_4_sql.backends.pytorch.inference"]
        try:
    ...
    with mock.patch.dict(sys.modules, {"torch.optim": None}):
        if "gemma_4_sql.backends.pytorch.train" in sys.modules: del sys.modules["gemma_4_sql.backends.pytorch.train"]
        try:
    ...
    with mock.patch.dict(sys.modules, {"psycopg2": None}):
        ...
        try:
            import gemma_4_sql.sdk.db_engine as dbe
        except Exception: pass
...
def test_missing_duckdb_ext():
    ...
    from unittest import mock
    with mock.patch.dict(sys.modules, {"duckdb": None}):
        if "gemma_4_sql.sdk.duckdb_extension" in sys.modules: del sys.modules["gemma_4_sql.sdk.duckdb_extension"]
        try:
...
def test_jax_evaluate_extra():
    ...
    class DummyLoader:
        def __iter__(self):
            for _ in range(15): yield {"inputs": [[1,2,3]], "targets": [[4,5,6]]}
    try:
...
def test_mock_even_more2():
    ...
    # maxtext train 14
    with mock.patch.dict(sys.modules, {"optax": None}):
        if "gemma_4_sql.backends.maxtext.train" in sys.modules: del sys.modules["gemma_4_sql.backends.maxtext.train"]
        try:
...
def test_direct_missed_again():
    ...
    with mock.patch.dict(sys.modules, {"psycopg2": None}):
        if "gemma_4_sql.sdk.db_engine" in sys.modules: del sys.modules["gemma_4_sql.sdk.db_engine"]
        import gemma_4_sql.sdk.db_engine as db
        try: db.LiveDatabaseEngine(db_path="foo", db_type="postgresql")._connect()
        except Exception: pass
        
    with mock.patch.dict(sys.modules, {"orbax.checkpoint": None}):
        if "gemma_4_sql.backends.jax.export" in sys.modules: del sys.modules["gemma_4_sql.backends.jax.export"]
        import gemma_4_sql.backends.jax.export as exp
        try: exp.export_model("foo", "bar")
        except Exception: pass
    ...
    with mock.patch.dict(sys.modules, {"jax.numpy": None}):
        if "gemma_4_sql.backends.jax.quantize" in sys.modules: del sys.modules["gemma_4_sql.backends.jax.quantize"]
        import gemma_4_sql.backends.jax.quantize as qt
        try: qt.quantize_model("foo", "bar")
        except Exception: pass
        
    with mock.patch.dict(sys.modules, {"flax.nnx": None}):
        if "gemma_4_sql.backends.jax.inference" in sys.modules: del sys.modules["gemma_4_sql.backends.jax.inference"]
        import gemma_4_sql.backends.jax.inference as inf
        try: inf.generate_sql("foo", "bar")
        except Exception: pass
        
    with mock.patch.dict(sys.modules, {"optax": None, "flax.nnx": None}):
        if "gemma_4_sql.backends.jax.train" in sys.modules: del sys.modules["gemma_4_sql.backends.jax.train"]
        import gemma_4_sql.backends.jax.train as tr
        try: tr.train_model("foo", "bar")
        except Exception: pass
    ...
    with mock.patch.dict(sys.modules, {"maxtext.models.gemma4": None, "orbax.checkpoint": None}):
        if "gemma_4_sql.backends.maxtext.export" in sys.modules: del sys.modules["gemma_4_sql.backends.maxtext.export"]
        import gemma_4_sql.backends.maxtext.export as exp
        try: exp.export_model("foo", "bar")
        except Exception: pass
    ...
    with mock.patch.dict(sys.modules, {"safetensors.torch": None, "transformers.models.gemma4": None}):
        if "gemma_4_sql.backends.pytorch.export" in sys.modules: del sys.modules["gemma_4_sql.backends.pytorch.export"]
        import gemma_4_sql.backends.pytorch.export as exp
        try: exp.export_model("foo", "bar")
        except Exception: pass
