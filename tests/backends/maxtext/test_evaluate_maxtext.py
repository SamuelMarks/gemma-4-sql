...
def test_evaluate_model_maxtext() -> None:
...
def test_evaluate_model_with_dataloader(monkeypatch) -> None:
    ...
    backend = (
        "tests/backends/maxtext/test_evaluate_maxtext.py".split("/")[-1]
        .replace("test_evaluate_", "")
        .replace(".py", "")
    )
    module_name = (
        f"gemma_4_sql.backends.{backend}_approach.evaluate"
        if backend != "maxtext"
...
def test_evaluate_model_maxtext_fallback(monkeypatch) -> None:
    from gemma_4_sql.backends.maxtext import evaluate
