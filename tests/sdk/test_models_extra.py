...
def test_train_from_scratch_pytorch(monkeypatch: object) -> object:  # type: ignore[return]
    ...
    monkeypatch.setattr(pt_train, "train_model", lambda **_kw: {"status": "mock"})  # type: ignore[attr-defined]
    res = train_from_scratch(model_name="mock", dataset="mock", backend="pytorch")
    if not res == {"status": "mock"}:
        raise AssertionError


def test_train_from_scratch_unknown() -> object:  # type: ignore[return]
