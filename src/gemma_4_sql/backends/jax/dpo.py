...
def _run_training_epochs(dataloader: object, epochs: int, policy_model: object, ref_model: object, optimizer: object, train_step: object) -> float:
    """Run training epochs."""
    final_loss = 0.0
    ...
    if jax is not None and jnp is not None and (jnn is not None) and (optax is not None) and (Gemma4ForCausalLM is not None):
        try:
            ...
            if dataloader is not None and hasattr(dataloader, "__iter__"):
                final_loss = _run_training_epochs(dataloader, epochs, policy_model, ref_model, optimizer, train_step)
            else:
