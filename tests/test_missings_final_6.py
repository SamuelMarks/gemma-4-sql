...
def test_true_missing():
    ...
    with mock.patch.dict(sys.modules, {"optax": None}):
        exec_import(
            "gemma_4_sql.backends.jax.train", "train_model", "a", "b", dataloader=[]
        )
    ...
    with mock.patch.dict(sys.modules, {"tensorflow": None}):
        ...
        exec_import(
            "gemma_4_sql.backends.keras.train", "train_model", "a", "b", dataloader=[]
        )
    ...
    with mock.patch.dict(sys.modules, {"optax": None}):
        exec_import(
            "gemma_4_sql.backends.maxtext.train", "train_model", "a", "b", dataloader=[]
        )
    ...
    with mock.patch.dict(sys.modules, {"torch.optim": None}):
        exec_import(
            "gemma_4_sql.backends.pytorch.train", "train_model", "a", "b", dataloader=[]
        )
