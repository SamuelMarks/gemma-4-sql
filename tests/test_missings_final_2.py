...
def reload_mod(mod_name, mocks, run_func=None):
    with mock.patch.dict(sys.modules, {m: None for m in mocks}):
...
def test_mock_even_more3():
    ...
    def run_eval(mod):
        ...
        try: mod.evaluate_model("foo", "bar", dataloader=DummyLoader())
        except Exception: pass
    reload_mod("gemma_4_sql.backends.jax.evaluate", [], run_eval)

    def run_exp(mod):
        try: mod.export_model("foo", "bar")
        except Exception: pass
    ...
    def run_keras_train(mod):
        try: mod.train_model("foo", "bar", dataloader=[])
        except Exception: pass
    ...
    reload_mod("gemma_4_sql.backends.keras.train", ["keras_nlp.models"], run_keras_train)

    reload_mod("gemma_4_sql.backends.maxtext.export", ["orbax.checkpoint"], run_exp)
    reload_mod("gemma_4_sql.backends.maxtext.export", ["maxtext.models.gemma4"], run_exp)
    reload_mod("gemma_4_sql.backends.maxtext.train", ["optax"], run_train)

    reload_mod("gemma_4_sql.backends.pytorch.export", ["transformers.models.gemma4"], run_exp)
    ...
    def run_db(mod):
        try: mod.LiveDatabaseEngine(db_path=":memory:", db_type="postgresql")._connect()
        except Exception: pass
