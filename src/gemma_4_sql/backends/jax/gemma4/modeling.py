...
class Gemma4RMSNorm(nnx.Module):  # type: ignore[misc]
    ...
    def __init__(self, dim: int, eps: float = 1e-06, *, with_scale: bool = True, dtype: jnp.dtype = jnp.float32, _shd: PartitionSpec | None = None, rngs: nnx.Rngs) -> None:
        ...
        self.dtype = dtype
        if self.with_scale:
            self.scale = nnx.Param(jax.nn.initializers.zeros(rngs.params(), dim, dtype=dtype))
        else:
            self.scale = None
...
class VisionConfig:
    ...
    patch_size: int = 14
    shd_cfg: VisionShardConfig = field(default_factory=VisionShardConfig.no_sharding)  # type: ignore[assignment]
...
class SiglipMLP(nnx.Module):  # type: ignore[misc]
    """MLP for SigLIP.
...
class SiglipEncoderLayer(nnx.Module):  # type: ignore[misc]
...
class ConstVar(nnx.Variable):  # type: ignore[misc]
...
class StatVar(nnx.Variable):  # type: ignore[misc]
...
class SiglipVisionTransformer(nnx.Module):  # type: ignore[misc]
...
class Gemma4MultiModalProjector(nnx.Module):  # type: ignore[misc]
...
def batched_merge_modalities(img_emb: Array, text_emb: Array, token_mask: Array) -> Array:
    ...
    def merge_modalities(i_emb: object, t_emb: object, mask: object) -> object:
        ...
        safe_indices = jnp.clip(img_indices, 0, i_emb.shape[0] - 1)  # type: ignore[attr-defined]
        aligned_images = i_emb[safe_indices]  # type: ignore[index]
        return jnp.where(mask[:, None], aligned_images, t_emb)  # type: ignore[index]
...
class LayerCache(nnx.Module):  # type: ignore[misc]
...
class ModelConfig:
    ...
    shd_cfg: ShardConfig = field(default_factory=ShardConfig.no_sharding)  # type: ignore[assignment]
...
class Gemma4MoE(nnx.Module):  # type: ignore[misc]
...
class Gemma4Attention(nnx.Module):  # type: ignore[misc]
    ...
    def __call__(self, x: Array, positions: Array, cache: LayerCache | None = None, attention_mask: Array | None = None) -> Array:
        ...
        structural_mask = jnp.where(mask, 0.0, -10000.0).astype(q.dtype)[:, None, :, :]
...
class Gemma4DecoderLayer(nnx.Module):  # type: ignore[misc]
    ...
    def __call__(self, x: Array, positions: Array, cache: LayerCache | None = None, attention_mask: Array | None = None, per_layer_input: Array | None = None) -> Array:
        ...
        if self.config.hidden_size_per_layer_input and per_layer_input is not None:
            ...
            x_ple = jax.nn.gelu(x_ple, approximate=True)
...
class Gemma4ForCausalLM(nnx.Module):  # type: ignore[misc]
    ...
    def from_pretrained(cls, model_name: str, config: ModelConfig | None = None) -> object:
        ...
        if config is None:
            ...
            config = config_map[model_name]()  # type: ignore[assignment]
    ...
    def __call__(self, input_ids: Array, positions: Array, cache: Cache | None = None, attention_mask: Array | None = None, pixel_values: Array | None = None, **kwargs: JSONValue) -> Array:
        ...
        ----
            ...
            **kwargs: Additional keyword arguments like image_token_mask, input_features, input_features_mask, audio_token_mask.
